...
export default function Home({ onNavigate }: Props) {
  ...
  const { recentlyPlayed, likedSongIds } = usePlayer();
  ...
  // Mood mixes with vector SVG icons
  const moodMixes = [
    {
      name: "Workout Energy",
    ...
    <div className="animate-fade-in px-4 md:px-8 py-6 space-y-10 pb-16">
      ...
      <section className="space-y-4">
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
          <div>
            <h1 className="text-3xl md:text-5xl font-black tracking-tight text-gradient-primary font-heading">
              {greeting}
            </h1>
          ...
          <div className="flex flex-wrap gap-2">
            {regionalMixes.map((mix) => (
              <button
                ...
                className={`px-4 py-2 rounded-full text-xs font-extrabold text-white bg-gradient-to-r ${mix.color} shadow-lg hover:scale-105 transition-transform`}
              >
                {mix.title}
              </button>
            ))}
        ...
        >
          ...
          <div className="relative z-10 flex flex-col md:flex-row items-start md:items-end gap-6">
            ...
            <div className="flex-1">
              ...
              <h2 className="text-3xl md:text-5xl font-black mt-2 text-white tracking-tight font-heading">
                {featuredPlaylist.name}
              </h2>
              <p className="text-white/80 mt-2 max-w-xl text-sm md:text-base leading-relaxed font-medium">
      ...
      <section className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div
          ...
          className="glass-card-premium p-5 rounded-3xl flex items-center justify-between cursor-pointer border border-rose-500/20 hover:border-rose-500/50 group"
        >
          <div className="flex items-center gap-4">
            ...
            <div>
              <h3 className="font-extrabold text-lg text-white font-heading group-hover:text-rose-400 transition-colors">Liked Songs</h3>
              <p className="text-xs text-white/60 font-medium">{likedSongIds.length} saved tracks</p>
            </div>
          </div>
          <button className="h-10 w-10 rounded-full bg-white/10 grid place-items-center text-white icon-btn-smooth">
            <svg viewBox="0 0 24 24" className="h-5 w-5" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round"><path d="M5 12h14M12 5l7 7-7 7"/></svg>
          </button>
        </div>

        <div
          onClick={() => onNavigate("search")}
          className="glass-card-premium p-5 rounded-3xl flex items-center justify-between cursor-pointer border border-[#18E29A]/20 hover:border-[#18E29A]/50 group"
        >
          <div className="flex items-center gap-4">
            <div className="h-14 w-14 rounded-2xl bg-gradient-to-br from-[#18E29A] to-[#6D5EF8] grid place-items-center text-black font-extrabold text-2xl shadow-xl">
              <svg viewBox="0 0 24 24" className="h-7 w-7 text-black" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
                <polygon points="13 2 3 14 12 14 11 22 21 10 12 10 13 2" />
              </svg>
            </div>
            <div>
              <h3 className="font-extrabold text-lg text-white font-heading group-hover:text-[#18E29A] transition-colors">Discover Weekly</h3>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          {moodMixes.map((m) => (
            <button
              key={m.name}
              onClick={() => onNavigate("search")}
              className={`p-5 rounded-3xl bg-gradient-to-br ${m.color} text-left text-white shadow-xl hover:scale-105 transition-transform flex flex-col justify-between h-32 border border-white/10`}
            >
              <div>{m.icon}</div>
              <span className="font-extrabold text-sm md:text-base font-heading">{m.name}</span>
            </button>
          ))}
        </div>
      </Section>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-4">
          {Array.from(new Set(songs.map((s) => s.genre).filter(Boolean) as string[])).map((genre, idx) => (
            <button
              key={genre}
              onClick={() => onNavigate("search")}
              className="p-5 rounded-3xl glass-card-premium text-left font-black text-base text-white hover:border-[#18E29A] transition-all font-heading"
              style={{
  ...
  return (
    <section className="space-y-3">
      <div className="flex items-center justify-between">
        <h2 className="text-xl md:text-2xl font-black text-white tracking-tight flex items-center gap-2 font-heading">
          <span>{title}</span>
        </h2>
      </div>
      {children}
...
function ScrollRow({ children }: { children: React.ReactNode }) {
  return <div className="flex gap-4 overflow-x-auto pb-4 -mx-2 px-2 snap-x no-scrollbar">{children}</div>;
}
...
function ArtistCard({ artist, onNavigate }: { artist: Artist; onNavigate: (view: string, id?: string) => void }) {
  return (
    <button
      onClick={() => onNavigate("search")}
      className="group relative text-left w-36 sm:w-44 shrink-0 glass-card-premium p-3.5 rounded-3xl transition-all"
    >
      ...
      <div className="mt-3 text-center font-extrabold text-sm text-white truncate font-heading">{artist.name}</div>
      <div className="text-[11px] text-center text-white/50 uppercase tracking-widest font-black mt-0.5">Artist</div>
    </button>
  );
}
...
function AlbumCard({ album, onNavigate }: { album: Album; onNavigate: (view: string, id?: string) => void }) {
  return (
    <button
      ...
      className="group relative text-left w-36 sm:w-44 shrink-0 glass-card-premium p-3.5 rounded-3xl transition-all"
    >
      <div className="relative overflow-hidden rounded-2xl aspect-square shadow-xl border border-white/10">
      ...
      <div className="mt-3 font-extrabold text-sm text-white truncate font-heading">{album.name}</div>
