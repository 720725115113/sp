...
export default function PlayerBar({
  ...
  if (!song) {
    return (
      <div className="fixed bottom-0 left-0 right-0 z-40 h-20 glass-card-premium border-t border-white/10 px-6 flex items-center justify-between text-sm text-white/50">
        <div className="flex items-center gap-3">
          <div className="h-10 w-10 rounded-xl bg-white/5 border border-white/10 grid place-items-center">
            <svg viewBox="0 0 24 24" className="h-5 w-5 text-white/30" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
              <path d="M9 18V5l12-2v13M9 9l12-2" />
              <circle cx="6" cy="18" r="3" />
              <circle cx="18" cy="16" r="3" />
            </svg>
          </div>
          <span className="font-medium">Select a song from the library to start streaming.</span>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="sticky bottom-0 z-40 w-full glass-card-premium border-t border-white/10 px-3 py-2.5 md:px-6 md:py-3.5 grid grid-cols-[1fr_auto] md:grid-cols-[1fr_1.4fr_1fr] items-center gap-2 md:gap-4 shadow-2xl backdrop-blur-3xl">
      {/* Left: song info & artwork */}
      <div className="flex items-center gap-3.5 min-w-0">
        <button
          onClick={onOpenNowPlaying}
          className="group relative h-12 w-12 md:h-14 md:w-14 rounded-2xl overflow-hidden shrink-0 border border-white/10 shadow-xl"
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-2">
            <button
              onClick={onOpenNowPlaying}
              className="truncate text-sm md:text-base font-extrabold font-heading text-white hover:text-[#18E29A] transition-colors text-left"
            >
              {song.title}
            </button>
        ...
        >
          <svg viewBox="0 0 24 24" className="h-4 w-4" fill={p.likedSongIds.includes(song.id) ? "currentColor" : "none"} stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
      ...
      <div className="flex flex-col items-center gap-1.5 min-w-0">
        <div className="flex items-center justify-center gap-3 md:gap-5">
          ...
          <button
            ...
            className={`h-9 w-9 md:h-10 md:w-10 grid place-items-center rounded-full icon-btn-smooth ${
              p.shuffle
          ...
          <button
            onClick={p.prev}
            aria-label="Previous"
            className="h-9 w-9 md:h-10 md:w-10 grid place-items-center rounded-full text-white/80 hover:text-white hover:bg-white/10 icon-btn-smooth"
          >
          ...
          >
            {p.isPlaying ? (
              ...
              <svg viewBox="0 0 24 24" className="h-6 w-6 text-black translate-x-[1.5px]" fill="currentColor">
          ...
          <button
            ...
            className={`h-9 w-9 md:h-10 md:w-10 grid place-items-center rounded-full relative icon-btn-smooth ${
              p.repeat !== "off"
                ? "bg-[#18E29A]/20 text-[#18E29A] border border-[#18E29A]/40 shadow-[0_0_15px_rgba(24,226,154,0.4)]"
                : "text-white/60 hover:text-white hover:bg-white/10"
            }`}
      ...
      <div className="hidden md:flex items-center justify-end gap-2 text-white/70">
        ...
        <div className="relative">
          <button
            ...
            className="px-2.5 py-1 rounded-xl text-xs font-bold bg-white/5 border border-white/10 hover:bg-white/10 text-white/80 hover:text-white transition-all"
            aria-label="Speed"
          >
            {p.playbackSpeed}x
          </button>
